...
#m220024
#Lab5 
#Resources: (1) hashit3.py; this program is hashit3.py but with a few line changes for the STDOUT
...
for root, directory, files in os.walk(BASEDIR):
    ...
    for check in IGNORE_DIR:
        if check.find(root) == 0 or root.find(check) == 0:
            PATH_OK = False
    if PATH_OK: #Changes were made to this portion of hashit3.py in order to make it a useful tool for a user. The changes made below 
        #are solely to the print statements in order to only show the user what the most recent modifications were in the file 
        #system - nothing more 
        print("Modification of File System Since Last Program Run:\n")
        for file in files:
            this_file = root + '/' + file
